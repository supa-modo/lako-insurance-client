...
const ApprovedApplicationsPage = () => {
  ...
  const [activeFilters, setActiveFilters] = useState({
    status: "approved", // Default filter for approved applications
    insuranceType: null,
    coverType: null,
    dateRange: null,
  });
  ...
  const fetchApplications = useCallback(async () => {
    ...
    try {
      const params = {
        ...
        limit: itemsPerPage,
        search: searchTerm,
        status: "approved", // Always filter for approved applications
      ...
      if (response && response.success) {
        ...
        setError(response?.message || "Failed to fetch approved applications");
      }
    } catch (err) {
      setError("Failed to fetch approved applications");
      console.error(err);
    } finally {
      setLoading(false);
    ...
    try {
      ...
      if (response && response.success) {
        ...
        const stats = {
          ...
          approvedApplications: applications.length, // All are approved
        };
        setStatistics(stats);
      }
    } catch (err) {
  ...
  const clearAllFilters = () => {
    setActiveFilters({
      status: "approved", // Keep the approved filter
      insuranceType: null,
      coverType: null,
      dateRange: null,
    });
  ...
  const getStatusColor = (status) => {
    return "bg-green-100 text-green-800 border-green-300"; // All are approved
  };
      ...
      <div className="h-[calc(100vh-64px)] flex flex-col overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
              ...
              <p className="text-gray-500 text-sm">
                View and manage approved insurance applications
              </p>
            </div>
            ...
            <div className="bg-white px-6 py-4 rounded-xl border border-gray-200 border-l-4 border-l-purple-500 shadow-md">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    KES {(statistics.totalPremiumValue || 0).toLocaleString()}
                  </p>
                </div>
                    ...
                    <tr>
                      ...
                      >
                        <div className="flex flex-col items-center">
                          <TbShieldCheck className="h-12 w-12 text-gray-300 mb-3" />
        ...
        <div className="bg-white border-t border-gray-200 px-6 py-3">
          <div className="flex flex-wrap justify-between items-center text-sm text-gray-600">
            <div>
              {filteredAndSortedApplications.length} Approved Applications
            </div>
